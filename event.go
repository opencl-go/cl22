...
// #include "api.h"
// extern cl_int cl22SetEventCallback(cl_event event, cl_int callbackType, uintptr_t *userData);
import "C"
import (
	"fmt"
	"unsafe"
...
func SetEventCallback(event Event, callbackType EventCommandExecutionStatus, callback func(error)) error {
	...
	status := C.cl22SetEventCallback(event.handle(), C.cl_int(callbackType), callbackUserData.ptr)
	if status != C.CL_SUCCESS {
...
//export cl22GoEventCallback
func cl22GoEventCallback(_ Event, commandStatus C.cl_int, userData *C.uintptr_t) {
